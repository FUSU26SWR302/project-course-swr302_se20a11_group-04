...
export default function Login() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      else if (roles.includes('ROLE_STAFF'))   navigate('/staff/tables');
      else if (roles.includes('ROLE_KITCHEN')) navigate('/kitchen/queue');
    ...
    <div className="min-h-screen flex bg-white">
      ...
      <div className="hidden lg:block w-[45%] relative overflow-hidden">
        ...
        <div className="absolute inset-0 bg-gradient-to-t from-black/60 via-black/10 to-transparent flex flex-col justify-end p-10">
          ...
          <p className="text-white/80 text-sm">Trải nghiệm món ăn đỉnh cao tại TP.Đà Nẵng</p>
        </div>
      </div>
      ...
      <div className="flex-1 flex items-center justify-center px-8 py-10">
        <div className="w-full max-w-md">
          <h1 className="text-2xl font-bold text-gray-800 mb-2 text-center">Đăng Nhập</h1>
          <p className="text-gray-400 text-sm text-center mb-8">Chào mừng trở lại! Vui lòng đăng nhập.</p>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="flex items-center justify-between text-xs">
              <p className="text-gray-500">
                Chưa có tài khoản?{' '}
                <Link to="/register" className="text-[#e85d04] font-semibold hover:underline">Đăng ký</Link>
              </p>
              <Link to="/forgot-password" className="text-[#e85d04] hover:underline">Quên mật khẩu?</Link>
            </div>
            ...
            <div className="relative flex items-center gap-3 my-1">
              ...
              <span className="text-xs text-gray-400 shrink-0">Hoặc đăng nhập bằng</span>
              <div className="flex-1 h-px bg-gray-200"></div>
            </div>
          ...
          <div className="mt-6 p-3 bg-gray-50 rounded-xl text-xs text-gray-400 text-center">
            Demo: đăng ký tài khoản mới để test
          </div>
        </div>
      </div>
    </div>
